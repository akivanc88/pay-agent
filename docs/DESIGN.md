...
| Policy gate (spend cap + destination allowlist + cumulative cap) | This project's own design | **Real** — halts *before* any draw; verifies the signed IntentMandate. A per-transaction cap **and** an opt-in cumulative ceiling summed across settled runs under the mandate's `jti`; the mandate's own expiry bounds the window (no independent rolling period — that's an explicit stretch). See M4.5 |
| Append-only audit trail (`run_events`) | This project's own design | **Real** — trigger-enforced append-only, like the ledger |
| Approvals (persist + decide-once + standing authorization) | This project's own design | **Real** — a decided approval cannot be flipped; a human may *explicitly opt in* to "trust this destination going forward", which reissues the IntentMandate (new `jti`, widened cap/allowlist, fresh expiry). Opt-in only — a plain approve never widens the mandate, and the model can never request it. See M4.5 |
...
| Cumulative-cap window (M4.5) | A budget usually resets on a calendar period ("$50/month") | The cumulative ceiling sums settled runs under the IntentMandate's `jti` and is bounded by the mandate's own `exp` | Reusing the mandate's validity window keeps the accounting to one signed object with no separate rolling-period state. A calendar-period budget that resets independently of the mandate is an explicit further stretch this milestone does not claim. |
...
- ~~The policy gate has no cumulative cap.~~ **Closed by M4.5** (`docs/PLAN.md`,
  ...
  "agents malfunctioning... impose unexpected costs on small businesses" concern raised
  against Google's UCP (retrieved 2026-08-07; see Sources) — a per-transaction cap alone
  bounds one bad call, not a runaway sequence of them.
- ~~An approval does not become a standing authorization.~~ **Closed by M4.5**
  (`docs/PLAN.md`, 2026-08-07) — an approval decision now has an explicit, opt-in second
  choice ("approve, and trust this destination going forward") that reissues the
  IntentMandate with a widened cap/allowlist and a fresh expiry. A plain approve still
  resolves only the one run; nothing widens a grant the human didn't ask to widen.
