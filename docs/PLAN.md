...
- **M4.5 — Standing authorization and a cumulative cap.** Closes the two gaps a user notices first — *don't make me re-approve the same bill; don't let one high cap become unlimited autonomous spend* — the parts the crypto-heavy M3 machinery doesn't cover, and the parts `docs/FINDINGS.md` names as the ones a user would notice missing first. It sits before M5 because it changes the same policy gate the real-card path and the brain both depend on; better to land it before the environment-locked, harder-to-revisit live-money milestone. *Demo: approve a StreamCo bill once with "trust this destination going forward", and the identical bill next cycle settles with no prompt; run three charges each under the per-transaction cap but summing past a smaller cumulative cap, and the third halts for approval even though it would have passed alone.*
  - **Done — a budget and a memory for the gate** *(2026-08-07, `apps/agent/src/orchestrator.ts` + `packages/{mandate,db}` + `apps/web`)*. **Standing authorization:** an approval decision gains an explicit, opt-in second choice — *"and trust `<destination>` up to `<amount>` going forward"* — surfaced as a checkbox next to Approve in the inbox and run timeline. When a human (never the model) chooses it, `resumeRun` **reissues** the run's gating IntentMandate as a brand-new signed mandate — new `jti`, allowlist widened to the destination, per-transaction cap raised to cover the approved amount, fresh expiry — and persists it before settling; a future run presenting it settles with no prompt. A plain approve leaves the mandate untouched: nothing silently escalates trust, and the brain's `resume_run` tool has no way to request it (AGENTS.md rule 26). **Cumulative cap:** `IntentClaims` gains an optional `cumulativeCapMinor`; the consent store sums settled-run amounts under a mandate's `jti` (`sumSettledAmountForMandate`), and the (still pure) `evaluatePolicy` refuses once running-total + new-amount would cross it, raising a new `over_cumulative_cap` approval reason — checked *in addition to* the per-transaction cap, before any draw. The inbox and run timeline show usage ("used $40.00 of $50.00 cumulative cap"). Mandates stay immutable (reissue mints a new one), and the audit trail stays append-only. Proven end to end by new orchestrator tests (approve-with-trust reissues a mandate that settles the next cycle with zero prompts; three under-cap charges, the third refused once the total would exceed the budget; a plain approve does *not* widen the mandate) and consent-store tests (the cumulative sum counts only settled runs gated by a given `jti`).
    - **Honestly scoped / simplified (see `docs/DESIGN.md`):** the cumulative window is the mandate's own `exp` — there is **no** independent rolling period; a calendar-period budget ("$50/month" that resets on its own) is a further stretch this milestone does not claim. `cumulativeCapMinor` is **opt-in**: an IntentMandate without it behaves exactly as M3/M4 did (per-transaction cap only), so the brain's `draft_intent` — whose surface is unchanged — still drafts un-budgeted intents; the cumulative ceiling is set explicitly (a reissue, a script, or a test). The reissued standing mandate is persisted and returned from `resumeRun`; wiring it as the user's *default* next authorization across a live web session (rather than presenting it explicitly) waits on the same real-auth/`demo-user` gap as the rest of the consent layer.
- **M5 — The real card** *(local only, recorded)*. Enrolled-balance-as-hint in the planner and the guarded live decline path. *Demo: your actual Visa gift card, a real over-balance attempt, a real network decline, and the agent recovering correctly.* The closing beat — everything before it could be simulation; this can't be.
  - **Done — the code path** *(2026-08-12, `apps/agent/src/{destination,planner}.ts` + `apps/store/src/payments/stripe.ts`)*. **Enrolled-balance-as-hint:** `CardFunding` gains an `enrolledBalanceMinor: Minor | null`, the same "hint, not a fact" shape as the gift card's `hintMinor` — the planner still authorizes the card leg in full regardless (a card is never drawn "up to" the way a gift card is), but `InstrumentPlan.cardLikelyExceedsEnrolledBalance` now labels a run honestly when the ask looks larger than the balance the user recorded, and the run log surfaces it ("likely to exceed the recorded card balance — expect a decline") before anything is attempted. **The guarded live path:** `assertAmountExceedsEnrolledBalance` is a pure guard — refuses outright unless the attempted amount exceeds the enrolled balance, so the only live outcome this project can ever produce is a decline. `attemptGuardedLiveDecline` wraps it around `liveClient()`; `captureAuthorization` is unreachable from it under any code path, and an authorization that unexpectedly succeeds (the recorded balance was wrong) is cancelled immediately and reported as `unexpectedly_authorized`, never treated as this demo's decline. `apps/store/scripts/live-decline-check.ts` (`pnpm --filter @pay-agent/store live-decline-check <pm_id> <balance> <amount>`) is the script that runs it. Proven by unit test exactly as verification item 9 asks — the guard is asserted directly (no live key, no network), and `attemptGuardedLiveDecline` is exercised through a `setLiveClient` test seam covering a refused sub-threshold amount, a genuine decline, and an unexpected authorization (cancelled, never captured).
    - **Honestly scoped / simplified:** this is the code path only. The physical run itself — a real Visa gift card enrolled locally, a genuine over-balance attempt, a real network decline captured and recorded — has **not** been executed; it needs a physical card and a local machine no automated session has, and stays exactly where `docs/PLAN.md`'s own scoping already put it: local only, run and recorded by hand.
- **M6 — Publish.** GitHub Pages write-up, deployed test-mode demo, recorded video.
  - **Done — a visual pass, two real bugs, and the deployed demo** *(2026-08-23)*. Before publishing, five independent fresh critics reviewed every surface (56 screenshots — desktop/mobile, light/dark) blind against `apps/web/DESIGN-CONTRACT.md`'s bar (real Stripe/Shopify/Apple Pay caliber, restrained — explicitly not a WebGL showpiece). Verdict: the app was already close to that bar. Two surfaces failed on concrete, fixable defects (stray list bullets on StreamCo; a leaked "(stub)" label and a floating-link layout bug in the activity inbox — the inbox finding also caught the screenshot harness itself quietly flooding the shared demo DB, since each viewport/theme context re-runs its click prep); the rest passed with small polish items (an amber "warning" border on a line that was actually reassuring, a few off-token font sizes, a stretched mobile thumbnail). All findings were re-verified against fresh screenshots after fixing, not just trusted. Along the way, two bugs unrelated to visuals turned up and were fixed: a schema-migration ordering bug that 500'd `/activity` on any consent DB created before M4.5, and a missing error boundary that let that bug (or any future one) crash the *entire* agent server instead of failing one request.
    - **Deployed**, not just built: `apps/store` and `apps/web`+`apps/agent` are live on Railway — [store](https://store-production-331d.up.railway.app), [web + Agent Console](https://web-production-5a199f.up.railway.app). Getting there surfaced three more real bugs, all fixed properly rather than routed around (full detail in `docs/DEPLOY.md`): a build command that skipped building workspace dependencies first; `@hono/node-server`'s `serve()` resolving to loopback-only inside a container (worked locally, 502'd every request once deployed); and the agent's demo-card minting shelling out to the store's local filesystem — silently fine on one laptop, silently wrong once agent and store became separate containers, and an actual violation of `AGENTS.md` rule 6 (agent/merchant separated by HTTP) that this one code path had quietly slipped. Fixed with a real testing-only HTTP endpoint on the store, not a workaround. `apps/web` and `apps/agent` deploy together, as two processes in one container, because they share one local consent DB by file path — documented in `docs/DEPLOY.md` under "Why web+agent share a container" rather than migrated to a networked DB, which would be its own stretch-goal-sized project.
    - **The write-up**: `site/index.html`, a single hand-built page styled with the same tokens as `apps/web` (not a Jekyll-default markdown dump), presenting the findings piece, the three-layer architecture, and the milestone history, deployed via `.github/workflows/pages.yml`. `docs/FINDINGS.md` itself needed a correction first — it was still claiming standing authorization and the cumulative cap as "not built, tracked as M4.5" after M4.5 had already shipped (2026-08-07); a findings piece that overstates its own remaining gaps is a smaller version of the same honesty failure as understating them, so it's fixed and the correction is dated rather than silent.
    - **The video**: a real, silent Playwright screen recording driven against the live public URLs (not localhost, not injected state — actual clicks: add to cart, a genuine Stripe test-mode decline that reverses the gift-card draw, a successful split settlement, the full instruct-to-pay flow including a pause-for-approval and the human approving it from the inbox). Not committed into the repository, to avoid bloating it with binary history — handed off directly rather than as a tagged GitHub Release, so hosting (YouTube, a release, elsewhere) is an open choice, not a foregone one.
    - **Honestly scoped / simplified:** the video is silent — no narration, captions only implicit in the on-screen UI itself, since this session has no audio synthesis available. The Railway `railway.json` config-as-code is single-service-per-file, so the repo carries two files under `deploy/` (`railway.store.json`, `railway.web-agent.json`) and a documented copy-before-deploy step rather than one canonical root config — `docs/DEPLOY.md` explains why, and names the cleaner one-time dashboard fix (per-service Config-as-code Path) that wasn't pursued because the CLI's own config-editing commands didn't reliably apply in this session. M5's physical real-card run remains exactly where it already was: local only, not something this pass touched.
...
9. **Live-mode guards (assert before ever running live):** live path refuses when `amount <= enrolledBalance`; live keys read only from `STRIPE_LIVE_SECRET_KEY`; no code path can reach a live client from a test-mode run; **the deployed build refuses to boot if a live key is present**. Verify by unit test, not by trying it. *(Done — all four. Three landed in M1 — `assertSafeStripeConfig` runs before the server binds a port, and the live client is a separate function checkout cannot reach. The `amount <= enrolledBalance` refusal landed with the live path itself in M5, 2026-08-12: `assertAmountExceedsEnrolledBalance` in `apps/store/src/payments/stripe.ts`, unit-tested directly with no live key and no network.)*
10. **Real-card run (M5, once, still outstanding):** enroll the Visa gift card via Elements, confirm no PAN appears in server logs or the DB, then attempt an over-balance charge and capture the genuine decline code. The code path this exercises (`apps/store/scripts/live-decline-check.ts`) is done; the physical run itself needs a local machine and a real card, which no automated session has — it stays yours to run.
